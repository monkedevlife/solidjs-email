...
import { defaultStyles, type StylesType } from './styles';
  ...
  const renderedHtml = () => {
    ...
    renderer.blockquote = ({ tokens }) => {
      ...
      const style = parseCssInJsToInlineCss(
        styles.blockQuote as Record<string, unknown>,
      );
    ...
    renderer.br = () => {
      const style = parseCssInJsToInlineCss(
        styles.br as Record<string, unknown>,
      );
    ...
    renderer.code = ({ text }) => {
      ...
      const style = parseCssInJsToInlineCss(
        styles.codeBlock as Record<string, unknown>,
      );
    ...
    renderer.codespan = ({ text }) => {
      const style = parseCssInJsToInlineCss(
        styles.codeInline as Record<string, unknown>,
      );
    ...
    renderer.del = ({ tokens }) => {
      ...
      const style = parseCssInJsToInlineCss(
        styles.strikethrough as Record<string, unknown>,
      );
    ...
    renderer.em = ({ tokens }) => {
      const text = renderer.parser.parseInline(tokens);
      const style = parseCssInJsToInlineCss(
        styles.italic as Record<string, unknown>,
      );
    ...
    renderer.heading = ({ tokens, depth }) => {
      ...
      const style = parseCssInJsToInlineCss(
        styles[styleKey] as Record<string, unknown>,
      );
    ...
    renderer.hr = () => {
      const style = parseCssInJsToInlineCss(
        styles.hr as Record<string, unknown>,
      );
    ...
    renderer.image = ({ href, text, title }) => {
      const style = parseCssInJsToInlineCss(
        styles.image as Record<string, unknown>,
      );
    ...
    renderer.listitem = ({ tokens }) => {
      ...
      const style = parseCssInJsToInlineCss(
        styles.li as Record<string, unknown>,
      );
    ...
    renderer.paragraph = ({ tokens }) => {
      const text = renderer.parser.parseInline(tokens);
      const style = parseCssInJsToInlineCss(
        styles.p as Record<string, unknown>,
      );
    ...
    renderer.strong = ({ tokens }) => {
      const text = renderer.parser.parseInline(tokens);
      const style = parseCssInJsToInlineCss(
        styles.bold as Record<string, unknown>,
      );
      return `<strong${style ? ` style="${style}"` : ''}>${text}</strong>`;
    };

    renderer.table = ({ header, rows }) => {
      const styleTable = parseCssInJsToInlineCss(
        styles.table as Record<string, unknown>,
      );
      const styleThead = parseCssInJsToInlineCss(
        styles.thead as Record<string, unknown>,
      );
      const styleTbody = parseCssInJsToInlineCss(
        styles.tbody as Record<string, unknown>,
      );
    ...
    renderer.tablecell = ({ tokens, align, header }) => {
      ...
      const style = parseCssInJsToInlineCss(
        styles.td as Record<string, unknown>,
      );
      const tag = align
    ...
    renderer.tablerow = ({ text }) => {
      const style = parseCssInJsToInlineCss(
        styles.tr as Record<string, unknown>,
      );
      return `<tr${style ? ` style="${style}"` : ''}>\n${text}</tr>\n`;
    };

    return marked.parse(props.children, {
      renderer,
      async: false,
    }) as string;
  };
